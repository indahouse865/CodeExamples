...
public class LargeInt
{
  ...
  public static LargeInt mul(LargeInt first, LargeInt second) {
	  LargeInt result = new LargeInt ("0");
	  LargeInt loss = new LargeInt("1");
//	  LargeInt zero = new LargeInt("+00");
	  String ab = second.toString();
//		  while (ab.charAt(1) != '0' || ab.charAt(ab.length() -1) != '0') {
	  boolean control = true;
		  while (control) {
		  int lent = ab.length() - 1;
		  int count = 0;
		  for (char ch:ab.toCharArray()) {
			  ...
			  if (lent == count && lent <= 5) {
			  control = false;
			  count = 0;
			  }
		  }
			  result = add(result, first);
			  second = subtract(second, loss);
			  ab=second.toString();
		  }
//	  
//		  I have spent the last 6 hours trying to make this work, however my initial idea of setting 
//		  up a base case did not work, since as the the large int grows, so too does the number of 0's in the 0 number.
//		  Initially it is +0, then once 2 digits numbers are reached it becomes +00, and so on. Once numbers reach the 
//		  thousands, additional commas are added in to separate the numbers cleanly. While aestetichally pleasing,
//		  it did not aid in me finding a solution at all.
//		  
//		  
//	  if (first.sign && first.sign == PLUS) { //determines the sing, 2 of the same size is always positive
//		  result.sign = PLUS;
//	  } else if (first.sign && first.sign == MINUS) {
//		  result.sign = PLUS;
//	  } else { //opposite signs is always negative
//		  result.sign = MINUS;
//	  } 
	result = subtract(result,first); //since it overincrememnts by 1, adds back one iteration
